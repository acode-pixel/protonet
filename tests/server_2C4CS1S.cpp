...
uv_thread_t tid1, tid2;
Client* client;
Client* client2;

void make_Reqs(void* args){
...
int main(int argc, char** argv){
    ...
    uv_fs_req_cleanup(&req);
    uv_fs_mkdir(server1->loop, &req, "./client2", O_RDWR, NULL);
    uv_fs_req_cleanup(&req);
    uv_fs_chmod(server1->loop, &req, "./client2", S_IRWXU | S_IXGRP | S_IRGRP | S_IROTH | S_IXOTH, NULL);
    uv_fs_req_cleanup(&req);
    ...
    client2 = new Client("Loopback Pseudo-Interface 1", "127.0.0.1", 5653, "client2", "./client2/");
    #else
    client = new Client("lo", "127.0.0.1", 5653, "client1", "./client/");
    client2 = new Client("lo", "127.0.0.1", 5653, "client2", "./client2/");
    #endif

    uv_fs_t req2;
    uv_thread_create(&tid1, make_Reqs, NULL);
    ...
    uv_fs_access(server1->loop, &req2, "./client2/test.txt", F_OK, NULL);
    ...
    if(req.result || req2.result){
        ...
        uv_fs_req_cleanup(&req);
        uv_fs_unlink(server1->loop, &req, "./client2/test.txt", NULL);
        uv_fs_req_cleanup(&req);
        uv_fs_rmdir(server1->loop, &req, "./client2", NULL);
        uv_fs_req_cleanup(&req);
